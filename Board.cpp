...
void Board::updateBoard() throw (gameOverExp){
	...
	if (mode == 0){
		...
		//cout<<"links: "<< numOfLinks << " posX: " << posX << " posY: " << posY << " tailPosX: " << tailCoorX << " tailPosY: " << tailCoorY << endl;
		//for (int i = 0; i < coorX.size(); i++)
		//cout << "coorX: " << coorX[i] << " coorY: " << coorY[i] << endl;
...
void Board::addLinkToSnake()
{
	...
	//cout << "adding link to coordinate  :" << "X - " << tailCoorX << " , " << "Y - " << tailCoorY << endl;
	//printBoard();
